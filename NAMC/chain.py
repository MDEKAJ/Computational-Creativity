# This file contains the creation of the Markov Chain/Process
# Version 1.3
# Author: Met Dekaj
...
class Chain:
    ...
    '''
    Trains the generator on a single file, or on a list of files, and saves the state to disk when finished using glob patterns.
    Returns the number of files successfully parsed and trained on.
    '''
    def bulk_train(self, path, verbose=False, batches=100, iterations=1000):
        i = 0
    ...
    '''
    This produces a sequence of words until a dead end or max length is reached.
    '''
    def generate(self, start_with=None, max_len=0, rand=lambda x: random.random() * x, verbose=False):
        ...
        if verbose:
            print('Generating a text of {0}, starting with "{1}":\n'
    ...
    '''
    Adjusts relationships between branches according to the fitness function.
    '''
    ...
    If verbose is true then shows a progress bar.
    '''
    ...
    '''
    Same function as generate, but with a nicer formatting.
    '''
